...
<?php
	...
	include_once 'inc/helper/class.pdowrapper.php';
?>
...
<div id="page-content">
    ...
    <!-- Your Content -->
    <h3 class="page-header page-header-top"><i class="icon-circle-blank"></i> Start here <small>Create your page</small></h3>
	
	<?php 
		// create new object of class wrapper
		$p = new PdoWrapper();
	
		$selectFields = array('projectId', 'shortDesc', 'longDesc', 'code');
		
		// select with where and bind param use select method
		$q = $p->select('project',$selectFields)->traceEnable()->results();
	?>
    ...
    <table id="example-editable-datatables" class="table table-bordered table-hover">
        <thead>
            <tr>
                ...
                <th><i class="icon-user"></i> Short Desc</th>
                <th class="hidden-phone"><i class="icon-envelope-alt"></i> Long Desc</th>
                <th class="hidden-phone"><i class="icon-file"></i> Code</th>
            </tr>
        </thead>
        <tbody>
